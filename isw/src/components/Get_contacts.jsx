...
const Get_contacts = (props) => {
    ...
    //console.log(props)
    const sesion_name = props.username
    ...
    const get_available = async() => {
        const devicesResponse = await fetch("http://localhost:5001/devices");
        const devicesData = await devicesResponse.json();
        const available = []
        devicesData.forEach((device) => {
            if (device.msc != "Disconnected"){
                available.push(device.device)
            }
        });
        ...
        try {
            ...
            for (const elem of jsondata) {
                if (!visited.includes(elem.origin_id) && !visited.includes(elem.recipient_id)) {
                    ...
                    if (availableList.includes(user_id[0].id)){
    ...
    return (
        <MDBCol>
            ...
            <MDBRow md="6" lg="5" xl="4" className="mb-4 mb-md-0 contact_list">
                ...
                {availableList.map(String).includes(sesion_id) ? (
                    <MDBCard>
                        <MDBCardBody>
                            <p style = {{padding: "10px"}}>{sesion_name}, te encuentras conectado</p>
                        </MDBCardBody>
                    </MDBCard>
                ) : (
                    <MDBCard>
                        <MDBCardBody>
                            <p style = {{padding: "10px"}} >Atención, te encuentras desconectado de la red</p>
                        </MDBCardBody>
                    </MDBCard>  
                )}
                </MDBCol>
                    ...
                    <MDBRow md="6" lg="7" xl="8">
                        ...
                        <MDBCol className="border border-dark">
                            <MDBTypography listUnStyled>
                                    ...
                                    if (msg.origin_id == sesion_id) {
                                        return (
                                            <div className="d-flex justify-content-end mb-4">
                                                <li className="d-flex justify-content-end mb-4" key={msg.id}>
                                                    <MDBCard className="justify-content-end w-100">
                                                        <MDBCardHeader className="d-flex justify-content-between p-3">
                                                            <p className="fw-bold mb-0">{sesion_name}</p>
                                                            <p className="text-muted small mb-0" style={{marginLeft: "40px" }}>{formatTimestamp(msg.timestamp)}</p>
                                                        </MDBCardHeader>
                                                        <MDBCardBody>
                                        ...
                                        return (
                                            <li className="d-flex justify-content-between mb-4" key={msg.id}>
                                                <MDBCard classname="justify-content-start w-100">
                                                    <MDBCardHeader className="d-flex justify-content-between p-3">
                                                        <p className="fw-bold mb-0">{msgreceptor.username}</p>
                                                        <p className="text-muted small mb-0"style={{ padding: "10px", marginBottom: "10px" }}>{formatTimestamp(msg.timestamp)}</p>
                                                    </MDBCardHeader>
                                                    <MDBCardBody>
                                                        <p className="mb-0">{msg.msg_content}</p>
                                                    </MDBCardBody>
                                ...
                                <Add_message onSend={handleSendMessage} originId={sesion_id} recipientId={msgreceptor?.id} />
                            </MDBTypography>
                        </MDBCol>
                        <MDBCol class="chat_spacer"></MDBCol>
                    </MDBRow>
